...
The cookie policy will describe exactly that. **No consent banner is required** at present, since only strictly-necessary storage is in use. If analytics is added later, this page and a consent mechanism must be revisited — noted as a forward dependency.
...
### 3.5 Analytics — out of scope, flagged

There is currently no way to measure whether any of this succeeds. Recommend adding privacy-friendly analytics (Plausible or Fathom — no cookies, no consent banner needed, keeps §2.6 simple) as a separate small decision. Not included here.
...
All four new pages reuse the existing nav, footer and CSS conventions so they are visually indistinguishable from the rest of the site.

| Page | Content |
|---|---|
| `security.html` | Ported verbatim from live: vulnerability reporting, `security@aiatella.com`, researcher commitments (no legal action in good faith, regular communication, PII protection), in/out of scope, 90-day critical resolution. Fixes the live bug where this page's `<title>` reads "Privacy" and its description reads "AIATELLA Privacy Page" |
| `cookies.html` | Written from verified behaviour (§2.6): functional `localStorage` only, no tracking cookies, no third-party trackers. Sections: what we use, why, how to clear it, what changes if we add analytics |
| `terms.html` | Draft per §2.5 — site use, IP, disclaimers, limitation of liability, governing law (Finland), changes to terms, contact. Visibly marked as awaiting legal review |
| `404.html` | Branded, nav + footer, links to the main sections. `noindex` |

### Fixes to existing pages

- `privacy.html` — repair the literal `**Effective Date:**` markdown; refresh the stale November 2024 date
- **Footer** — currently links only Privacy, and there is no `<footer>` element anywhere on the site. Add Security, Terms and Cookies links. Applies to all pages including blog details
...
1. **Indexability** — assert exactly the intended `robots` value on all 12 pages
...
4. **Heading structure** — exactly one logical `<h1>` per page; no heading level skipped
...
9. **Link integrity** — no internal `href` 404s, including the new footer links, across all 23 pages (12 top-level + 11 blog details)
...
| Analytics | Decision on whether to add privacy-friendly analytics (§3.5) |
| Forms | Framer API dependency must be resolved before cutover — tracked outside this spec |
